...
sed -i "s/^worker_count *=.*/worker_count = ${NEW_COUNT}/" vm_config.auto.tfvars
...
echo -e "\nworker-${NEW_COUNT} ansible_host=${NEW_WORKER_IP} ansible_user=ubuntu ansible_ssh_pass=ubuntu123 ansible_become_pass=ubuntu123" >> ansible/inventory/hosts.ini

# Step 7: Join the worker to the cluster
ansible-playbook -i ansible/inventory/hosts.ini ansible/k8s-setup.yml --limit worker-${NEW_COUNT}
